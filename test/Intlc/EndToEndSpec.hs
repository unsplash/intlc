...
spec = describe "end-to-end" $ do
  ...
  it "compiles plurals" $ do
    ...
    [r|{ "prop": { "message": "Age: {age, plural, =0 {newborn called {name}} =42 {magical} other {boring #}}", "backend": "tsx" } }|]
      =*= "import * as React from 'types/React'\nexport const prop: (x: { age: number; name: string }) => React.ReactElement = x => <>Age: {(() => { switch (x.age) { case 0: return <>newborn called {x.name}</>; case 42: return <>magical</>; default: return <>boring {new Intl.NumberFormat('en-US').format(x.age)}</>; } })()}</>"
  ...
  it "TypeScriptReact backend" $ do
    [r|{ "f": { "message": "{x} <z>{y, number}</z>", "backend": "tsx" } }|]
      =*= "import * as React from 'types/React'\nexport const f: (x: { x: string; z: (x: React.ReactElement) => React.ReactElement; y: number }) => React.ReactElement = x => <>{x.x} {x.z(<>{new Intl.NumberFormat('en-US').format(x.y)}</>)}</>"
